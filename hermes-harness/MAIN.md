...
содержательная часть не менялась. Скилл ссылается на конкретные примеры
файлов (`tasks/1.33/index.html`, `tasks/2.28/index.html`) — они живут в
рабочем проекте пользователя, а не в этом репозитории; это ожидаемо
(это не общий Hermes-скилл, а личный, привязанный к конкретной
физической/учебной практике пользователя), но стоит иметь в виду: если
агент когда-нибудь применит этот скилл в проекте, где этих файлов нет,
ссылки на примеры не сработают — сам текст инструкции при этом
достаточен и без них.
