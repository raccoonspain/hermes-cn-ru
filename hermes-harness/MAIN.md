...
**Важная находка про CPU (D-023, 2026-08-05):** контейнер-песочница
физически ограничен **1.0 vCPU** (`docker inspect hermes-<id> --format
'{{.HostConfig.NanoCpus}}'` → `1000000000` = 1.0 CPU), несмотря на то,
что `nproc` **внутри** контейнера рапортует 8 — это видимые хосту ядра,
не выделенная контейнеру доля. Любой инструмент/библиотека, которая
сама решает завести N потоков по числу `nproc`/`os.cpu_count()` (как
делал стоковый `config.yaml` пакета `rapidocr-onnxruntime`, ставя
`intra_op_num_threads: -1`), на деле создаёт много потоков, дерущихся
за квоту в одно ядро — тяжёлая просадка производительности (конкретный
случай: 71с → 17.5с на одном OCR-вызове после форсирования 1 потока,
см. `hermes-harness/scan-pdf-vision-ocr/models/*/config.yaml`).
**Практический вывод для любого будущего CPU-тяжёлого инструмента в
этой песочнице**: не доверять автоопределению числа потоков по
`nproc`/`cpu_count()`, явно ограничивать 1 (или проверять реальную
квоту через `docker inspect`, если доступ есть); и **не пытаться
компенсировать это конкурентными процессами** — тот же 1-vCPU лимит
общий на весь контейнер, параллельные вызовы просто делят одно ядро
на большее число частей и получаются медленнее, не быстрее (проверено:
4 параллельных вызова ~114с каждый против 4 последовательных по 17.5с).
Подробности — D-023 в `docs/decisions.md`.
