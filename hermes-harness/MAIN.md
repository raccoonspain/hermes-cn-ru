...
**Ретраи на уровне ниже модели (`agent.api_max_retries`) — поднято
2026-08-05 (D-021):**
...
Каждый вызов модели (любой — не только vision) обёрнут ретраем с
растущей паузой (`5с → 10с → 20с → 40с → 80с → потолок ~120с`,
`agent/retry_utils.py`). Дефолт — 3 попытки, то есть капитуляция
примерно за минуту при реальном исчерпании кредитов подписки (тариф
Payed, wormsoft.ru — 3 000 000 кредитов каждые 4 часа, `docs/
wormsoft-api.md`). 130 попыток покрывают ≈4 часа — одно окно сброса.
Не относится к самому `auxiliary.vision.timeout` выше (это таймаут
одного HTTP-запроса, отдельная настройка) — это число повторов
целиком неудачного вызова. Полная методология — D-021.
...
|---|---|---|---|---|
| `ocr-and-documents` | `productivity` (bundled Hermes — см. примечание ниже, v2.3.0→2.8.0) | [hermes-harness/ocr-and-documents/SKILL.md](./ocr-and-documents/SKILL.md) | 2026-08-03, правлен 2026-08-07 (v2.8.0, D-035) | Главная «лестница» выбора инструмента для текста из PDF/сканов — раньше знала только `pymupdf` (без OCR) и `marker-pdf` (3-5 ГБ, часто непрактично в песочнице), из-за чего единственным реальным способом прочитать скан было дорогое `vision_analyze`. Добавлена третья строка в таблицу и отдельный раздел — `rapidocr-onnxruntime` как средний, лёгкий (~30 МБ) вариант по умолчанию для чистого текста. **v2.5.0** — короткая отсылка к backoff-лестнице в `scan-pdf-vision-ocr` при сбое `vision_analyze`. **v2.6.0 (D-022)** — убрана неверная строка «Cyrillic out of the box» (дефолтная модель rapidocr физически не может её выводить), заменена предупреждением + ссылкой на bundled-кириллическую модель в `scan-pdf-vision-ocr`. **v2.7.0 (D-023)** — рекомендация переписана на вызов через `scripts/ocr_page.py` (не голый `RapidOCR()`) — заодно даёт 4x на потоках, не только кириллицу. **v2.8.0 (D-035)** — цифры про потоки/vCPU обновлены (1→4 потока, контейнер реально 4 vCPU после D-034 — старое «1 vCPU» было багом застрявшего persistent-контейнера, не лимитом). |
| `scan-pdf-vision-ocr` | `productivity` (self-authored Hermes, v1.0.0→v1.14.1) | [hermes-harness/scan-pdf-vision-ocr/SKILL.md](./scan-pdf-vision-ocr/SKILL.md) | 2026-08-03…2026-08-07, последний раз v1.14.1 (D-035, живой прогон) | **v1.14.1 (D-035, живой прогон 2026-08-07)** — живой прогон реального документа (4 стр. ДИ) подтвердил v1.14.0 на практике (~12–14с/страница, без сбоев), но вскрыл мелкий питфол: агент перед OCR перепутал хостовый путь скилла (`/home/hermes/.hermes/skills/...`) с реальным путём внутри песочницы (`/root/.hermes/skills/...`, `HOME=/root`, D-020). Добавлено явное предупреждение (заголовочный callout + пункт в Pitfalls), только `SKILL.md`, поведение скриптов не менялось. **v1.10.0→v1.13.1 — накопительные self-authored правки агента между 5 и 7 августа, без нашего участия** (тот же паттерн, что v1.3.0→v1.5.0 ниже) — детальный список изменений не восстановлен, зафиксирован только факт разрыва между последней нашей синхронизацией (v1.9.0) и версией на сервере на момент правки D-035 (v1.13.1, сверено `diff` — идентична локальной копии, значит уже когда-то ранее синхронизирована целиком, просто не залогирована здесь построчно). **v1.14.0 (D-035, 2026-08-07)** — потоки rapidocr подняты с 1 до 4 (`intra_op_num_threads`/`inter_op_num_threads` в обоих bundled-конфигах), контейнер-песочница реально на 4 vCPU (D-034 починил стоявший с момента создания баг застрявшего persistent-контейнера — было 1.0 vCPU, не лимит). Перемерено: 1 поток 27.3с, 4 — 11.6с, -1/auto — 19.8с, вывод везде идентичен. Все упоминания «1.0 vCPU»/«71с→17.5с» в скриптах и SKILL.md обновлены с честной пометкой «superseded», старые числа не стёрты. v1.1.0 — «Step 2.5», rapidocr до `vision_analyze`. **v1.2.0 (D-019)** — переписан раздел про передачу картинки в `vision_analyze`: старый скилл советовал base64+ресайз до 640×640 из-за якобы «потолка ~500 КБ», чего в реальном коде инструмента нет. Теперь скилл однозначно рекомендует путь/`file://` по умолчанию. **v1.3.0→v1.5.0** — накопительные self-authored правки агента между 3 и 5 августа (без нашего участия): новые `references/embedded-jpeg-rotation.md`, `bilingual-parallel-text-book.md`, `galanz-aerogril-session.md`, `graph-curve-extraction.md`, резюмируемый `scripts/batch_ocr_kirik.py` — локальная копия в этом репозитории успела отстать от сервера, синхронизирована целиком при этой правке (2026-08-05), не переписана заново. **v1.6.0 (D-021)** — раздел «Provider degraded» переписан: backoff-лестница `30с→1м→5м→15м→30м→1ч→2ч` с обязательной записью в чат вместо старого «повтори один раз»; явный запрет полагаться на суррогатные сигналы (% белых пикселей, ASCII ink-band) вместо настоящей vision-проверки при недоступности vision. **v1.7.0** — агент self-authored раздел про гомоглифы (`references/cyrillic-homoglyph-recovery.md`) после реального инцидента (юридический скан), синхронизирован в этот репозиторий. **v1.8.0 (D-022)** — найден корень проблемы гомоглифов (дефолтная rec-модель rapidocr — китайско-английская, кириллицы в словаре нет физически, не «путается», а не может её вывести); поставлена и провалидирована кириллическая модель (`models/rapidocr-cyrillic/`, с исправленным off-by-one багом в скачанном словаре); Step 2.5 переписан — модель выбирается по языку документа; гомоглиф-рецепт оставлен как fallback, не основной путь. **v1.9.0 (D-023)** — агент между нашими правками self-authored ошибочный «CWD-баг» и `templates/cyrillic_config.yaml` как workaround; live-проверка показала, что бага не было (`update_model_path()` резолвит Det/Cls всегда относительно папки пакета, не CWD) — исправлено на честное «CORRECTED», шаблон помечен superseded. Найден реальный лимит — контейнер-песочница на 1.0 vCPU (`docker inspect`), а bundled config.yaml стоял на `intra_op_num_threads: -1` (авто = 8 потоков по видимым `nproc` ядрам) — форсирование в 1 поток дало 71с→17.5с на полной странице, побайтово идентичный вывод; конкурентные вызовы **не** помогают (тот же лимит на весь контейнер). Новый `scripts/ocr_page.py` — точка входа по умолчанию (терсный stdout, полный текст только в файл — правило против раздувания контекста растущей сессии, которое, по имеющимся косвенным данным, съедает больше кредитов, чем сам OCR). Новый `models/rapidocr-latin/config.yaml` — тот же thread-фикс для нероссийских документов. |
| `scanned-document-recovery` | `productivity` (self-authored Hermes, v1.0.0→v1.2.0) | [hermes-harness/scanned-document-recovery/SKILL.md](./scanned-document-recovery/SKILL.md) | 2026-08-03, правлен 2026-08-05 (v1.2.0, D-021) | Условие срабатывания уточнено: раньше эту (дорогую, идёт искать зеркала в интернете) заглушку могло вызвать «rapidocr не пробовали», теперь — только «rapidocr реально дал пустой/битый результат на этом документе». **v1.2.0** — исправлено прямое противоречие со Слоем 2: старый текст советовал «vision молчит >60с — считай сбоем, ищи зеркало», конфликтовало с новой многочасовой лестницей; уточнено, что поиск зеркала — legitimate первый ход при вероятном существовании зеркала, не обход лестницы. |
| `series-task-workflow` | без категории, верхний уровень `~/.hermes/skills/` (self-authored Hermes) | [hermes-harness/series-task-workflow/SKILL.md](./series-task-workflow/SKILL.md) | Впервые смирроен и правлен 2026-08-05 (D-021) | Ранее не отслеживался в этом репозитории вообще (агент создал сам, три-файловый паттерн резюмируемости между сессиями). Добавлен новый раздел «Unattended continuation» — самопланирование через `hermes cron create` (расписание 4ч10м, `--deliver origin`, `--workdir`, `--repeat 5`) на случай, если задача явно ночная/неприсутственная или backoff-лестница исчерпана — раньше в этом случае агент просто останавливался и ждал, пока пользователь не напишет сам. |
...
|---|---|---|---|
| Собственный writable-слой контейнера | `/usr`, `/etc`, всё, что ставит `apt` | Только в самом контейнере (Docker overlay) | **Нет.** Новый контейнер = чистый образ `nikolaik/python-nodejs` заново. `chromium`, `libreoffice`/`poppler-utils`/`libgl1` (раздел 3, D-020) и шрифты придётся ставить заново. |
| Per-профильный `/root` | `/root` | `~/.hermes/sandboxes/docker/<профиль>/home` на хосте (bind mount) | **Да.** Новый контейнер того же профиля примонтирует ту же хостовую папку — `pip`-пакеты из `/root/.local` (Playwright, weasyprint, rapidocr, openpyxl/pandas/markitdown/python-docx и т.д.) переживут пересоздание — **но только если команда реально получила `HOME=/root`, см. ниже.** |
| Рабочая папка проекта | `/workspace` | `/home/hermes/workspace` на хосте (bind mount) | **Да.** Файлы агента/проектов не теряются. |
| Skills | `/root/.hermes/skills` (read-only) | `~/.hermes/skills` на хосте, аккаунт `hermes` | **Да**, и даже не зависит от контейнера вообще — это файл на хосте, монтируется в любой контейнер заново. Это касается и бинарных ассетов внутри папки скилла, не только `SKILL.md`/скриптов — например, кириллическая ONNX-модель под `scan-pdf-vision-ocr/models/rapidocr-cyrillic/` (D-022) пережила бы пересоздание контейнера точно так же, и уже подтверждена видимой из **обоих** контейнеров-песочниц без ручного копирования в каждый. |
...
**Важная поправка (D-020, 2026-08-05):** до этой правки durable-путь
`/root` из таблицы выше был скорее теоретическим для команд, которые
реально печатает модель — `tools/environments/docker.py` (наш
`terminal.backend: docker`) никогда не выставлял `HOME`, в отличие от
`environments/local.py`. Модель получала дефолт контейнера `HOME=/`, не
`/root`, и `pip install --user`/`~/.local` промахивались мимо durable-
пути — отсюда регулярные переустановки одного и того же пакета в новом
проекте/чате. Исправлено штатным `terminal.docker_env: {HOME: /root}` в
`config.yaml` — теперь `HOME=/root` гарантирован для всех команд с
момента инициализации контейнера. Подробности и как проверять — D-020 в
`docs/decisions.md`.

**Важная находка про CPU, дважды пересмотренная (D-023 2026-08-05 → D-034
2026-08-06 → D-035 2026-08-07):** контейнер-песочница **сейчас реально на
4 vCPU** (`docker inspect hermes-<id> --format
'{{.HostConfig.NanoCpus}}'` → `4000000000`, подтверждено и изнутри —
`cat /sys/fs/cgroup/cpu.max` → `400000 100000`). Так было не всегда: D-023
измерил и задокументировал **1.0 vCPU** как данность, но D-034 нашёл, что
это был баг, не лимит — `config.yaml` требовал 4 vCPU с самого D-004, оба
persistent-контейнера песочницы просто застряли на 1.0 CPU с момента
создания (`docker run --cpus` фиксируется один раз; `container_persistent:
true` не даёт пересоздаться) и никогда не подхватывали конфиг задним
числом. Починено `docker update --cpus 4` + новым скриптом
`hermes-sandbox-cpu-sync.sh` (`ExecStartPre` в `hermes-gateway.service` —
переприменяет лимит на каждом рестарте шлюза, не полагаясь на то, что
контейнер никогда не пересоздастся). Любой инструмент/библиотека, которая
сама решает завести N потоков по числу `nproc`/`os.cpu_count()` (как делал
стоковый `config.yaml` пакета `rapidocr-onnxruntime`, ставя
`intra_op_num_threads: -1`), всё ещё создаёт больше потоков, чем реальных
ядер (`nproc` внутри контейнера рапортует 8 — видимые хосту ядра, не
выделенная контейнеру доля, это не изменилось) — но правильное число для
форсирования теперь **4, не 1** (перемерено D-035: 1 поток 27.3с, 4 —
11.6с, -1/auto — 19.8с на том же тесте; см.
`hermes-harness/scan-pdf-vision-ocr/models/*/config.yaml`).
**Практический вывод для любого будущего CPU-тяжёлого инструмента в этой
песочнице**: не доверять автоопределению числа потоков по
`nproc`/`cpu_count()`, явно ограничивать реальной квотой (сейчас 4,
`docker inspect`/`cpu.max` — источник правды, не `container_cpu` в
`config.yaml` — на живых persistent-контейнерах эти два числа могут
разойтись, как уже было). Конкурентные процессы (несколько движков/
вызовов разом) больше не бесполезны, как при 1 ядре — но и не бесплатны:
дают скромный выигрыш (~18% на batch OCR, D-035), недостаточный, чтобы
оправдать сложность оркестрации, и едят общий бюджет
`pids.max=256` (отдельный, не CPU-шный риск `pthread_create failed`).
Подробности — D-023, D-034, D-035 в `docs/decisions.md`.
...
## 6. Ревизия всех скиллов, крутящихся на агенте

**Часть (а) закрыта 2026-08-05 (D-020), другим методом, чем планировалось
3 августа.** Вместо ручного прохода по каждому из 76 `SKILL.md`
(`apple` 4, `autonomous-ai-agents` 5, `creative` 17, `education` 2,
`email` 1, `github` 6, `media` 3, `mlops` 5, `note-taking` 1,
`productivity` 13, `research` 5, `smart-home` 1, `social-media` 1,
`software-development` 11) — прочитан `~/.hermes/skills/.usage.json`:
Hermes сам считает `use_count`/`last_used_at` с момента установки (31
июля). Реально использовались **13 из 76** (7 self-authored + 6
bundled, полный список — D-020 в `docs/decisions.md`). Список
«использовалось / нет» по всем 76 передан пользователю; **выключение —
штатным веб-дашбордом Hermes** (`hermes skills config`, интерактивно),
руками пользователя, не автоматизировано нами. Отдельно удалены (не
через дашборд — там не видны) 4 скилла `apple/*`: физически на диске,
но не зарегистрированы в `hermes skills list`, и `"os": ["darwin"]` —
100% нерабочие на Linux VPS.

**Уточнение состава по факту (не совпадает с тем, что предполагалось 3
августа):** self-authored (`source: local`) — только 7: `school-task-
analyzing`, `scan-pdf-vision-ocr`, `scanned-document-recovery`,
`local-browser-rendering`, `html-page-bundle`, `multi-task-analysis-
series`, `series-task-workflow`. `ocr-and-documents` — **bundled**, не
self-authored, как ошибочно записано в D-018/разделе 2.4 выше (пока не
исправлено там, история сохранена как есть, поправка — здесь и в
D-020).

**Часть (б) — сделана точечно, не по всем 76, а по факту того, что
пользователь оставил включённым сверх используемого:** `powerpoint`/
`xlsx`. Прочитаны их Prerequisites, найден и закрыт конфликт с клеткой
(оба требуют LibreOffice, скилл пишет `sudo apt install` — у `hermes`
нет sudo) — полная методология и сквозная проверка результата
(`recalc.py`/`soffice.py`+`pdftoppm` самих скиллов, не просто импорт) —
D-020. Заодно там же найдена и закрыта смежная первопричина (`HOME` не
долетал до durable `/root` в `docker`-backend терминала, из-за чего
любой pip-пакет — не только под pptx/xlsx — переустанавливался с нуля в
каждом новом чате).

**Остаётся не сделанным:** пункт (б1)/(б2) из исходного плана — для
оставшихся ~60 скиллов, которые пользователь ещё не решил включать
(в основном `mlops`/`research`/`github`/`creative`/остаток
`software-development` — см. D-020) периметр клетки не проверялся, и не
обязательно нужен: они либо выключены пользователем, либо ждут его
решения. Если пользователь позже включит что-то из этого списка —
повторить (б1)/(б2) точечно для конкретного скилла тем же методом, что
для `powerpoint`/`xlsx` (прочитать Prerequisites, сверить с
ограничениями D-003/D-004, поставить недостающее тем же admin-путём,
что D-017/018/020, зафиксировать здесь в разделе 2.4).
