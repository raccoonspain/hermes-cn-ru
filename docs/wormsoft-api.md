...
**Внутри одной цепочки цены моделей разные, иногда сильно** (это не
«примерно одинаково», хоть и может так казаться на первый взгляд).
Пример — цепочка `wormsoft/agent/high`:

| Модель | Вход | Выход | Кэш |
|---|---|---|---|
| minimaxai/minimax-m3 | 1 100 000 | 2 000 000 | 300 000 |
| kimi/kimi-k2.6 | 250 000 | 1 500 000 | 35 000 |
| zai/glm-5.1 | 900 000 | 3 500 000 | 100 000 |
| *(сам алиас `wormsoft/agent/high`)* | 650 000 | 3 400 000 | 20 000 |

Вход у minimax-m3 дороже входа у kimi-k2.6 в 4,4 раза, кэш — в 8,6 раза.
Это неважно для бюджета: биллинг всегда идёт по **фиксированной цене
самого алиаса** (нижняя строка), а не той модели, что фактически ответила
— см. «System Aliases» выше и D-001/D-007 в `docs/decisions.md`. Разброс
цен внутри цепочки влияет разве что на **поведение** (например, у
minimax-m3 контекст 1M токенов, у kimi-k2.6 — только 260K), не на цену.
