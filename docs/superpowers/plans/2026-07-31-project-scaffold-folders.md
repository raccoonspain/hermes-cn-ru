...
3. **Живая приёмка** — одноразовый `qa_temp` (создание/удаление как в
   D-015, см. `docs/changelog.md` за 2026-07-31). **Проверять на диске
   сервера (`ls -la` по SSH), не через дерево `project-workspace.html`**
   — финальное ревью (2026-07-31) показало: `workspace.list_tree` и
   фронтенд рисуют заголовки всех трёх бакетов безусловно, независимо от
   того, существуют ли папки реально (`tree[bucket]`/`tree[f"{bucket}_dirs"]`
   отдаются даже для отсутствующей директории — `os.path.isdir` там
   охраняет только `os.walk`, а не сам факт вывода бакета). Поэтому
   проверка через UI-дерево — ложно-зелёная: она не отличит успешный
   деплой от сорванного. Создать проект через «Быстрый чат», сразу
   зайти по SSH и подтвердить `ls -la` реальное наличие `source/outer/result`
   рядом с `about.md`/`AGENTS.md`/`history.md`; открыть существующий
   (`dem`/`rost`) проект, созданный до этой фичи — папки должны появиться
   на диске при открытии (тоже проверить `ls`, не только UI).
